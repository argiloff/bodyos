...
import { mealTypeLabel, useThemePalette } from "../../theme";
...
export default function PlanDetailScreen() {
  ...
  const { getPlanById, getResolvedRecipe, getGroceryList, deletePlan } =
    useData();
  ...
  const grocery = getGroceryList(plan.id);
  ...
  return (
    ...
    >
      <Text style={[styles.title, { color: theme.text }]}>
        Plan {plan.start_date} bis {plan.end_date}
      </Text>
        ...
        {days.map((day) => (
          ...
          >
            ...
            {grouped[day]
              ...
              .sort(
                (a, b) =>
                  (mealOrder[a.meal_type] ?? 99) -
                  (mealOrder[b.meal_type] ?? 99),
              )
              .map((meal) => {
                const recipe = getResolvedRecipe(meal.recipe_id);
                return (
                  <Pressable
                    key={`${day}-${meal.meal_type}-${meal.recipe_id}`}
                    style={[styles.mealRow, { backgroundColor: theme.cardAlt }]}
                    onPress={() => router.push(`/cook/${meal.recipe_id}`)}
                  >
                    <Text style={[styles.mealType, { color: theme.accent }]}>
                      {mealTypeLabel(meal.meal_type)}
                    </Text>
                    <Text style={[styles.mealName, { color: theme.text }]}>
                      {recipe?.name ?? "Rezept nicht verfügbar"}
                    </Text>
                  </Pressable>
                );
              })}
      ...
      >
        ...
        {grocery.map((item) => (
          <View
            key={item.product_id}
            style={[styles.groceryRow, { borderBottomColor: theme.border }]}
          >
            <View>
              <Text style={[styles.groceryName, { color: theme.text }]}>
                {item.name}
              </Text>
              <Text style={[styles.groceryMeta, { color: theme.muted }]}>
                {item.category}
              </Text>
            </View>
            <Text style={[styles.groceryAmount, { color: theme.text }]}>
              {item.amount_g} g
            </Text>
          </View>
        ))}
