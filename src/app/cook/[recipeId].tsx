...
export default function CookModeScreen() {
  ...
  const { getResolvedRecipe, products } = useData();
  ...
  const steps = recipe.steps.length
    ...
    : [{ instruction: "Keine Kochschritte hinterlegt", image_uri: undefined }];

  const stepProgress = Math.max(
    0,
    Math.min(100, ((step + 1) / Math.max(steps.length, 1)) * 100),
  );

  const ingredientRows = recipe.ingredients.map((ingredient) => {
    const product = productsById[ingredient.product_id];
    return {
      key: `${ingredient.product_id}-${ingredient.amount_g}`,
      name: product?.name ?? ingredient.product_id,
      ...
      >
        ...
        >
          <View
            style={[
              styles.progressFill,
              { backgroundColor: theme.accent, width: `${stepProgress}%` },
            ]}
          />
        </View>
        ...
        {steps[step].image_uri ? (
          <Image
            source={{ uri: steps[step].image_uri }}
        ...
        <View style={styles.row}>
          ...
          >
            <Text style={[styles.buttonText, { color: theme.accentText }]}>
              {step + 1 === steps.length ? "Fertig" : "Weiter"}
            </Text>
          </Pressable>
        </View>
      </View>
        ...
        {ingredientRows.map((item) => (
          <Pressable
            ...
            style={[
              ...
              checked[item.key]
                ? [styles.ingredientChecked, { borderColor: theme.accent }]
                : undefined,
            ]}
            onPress={() =>
              setChecked((prev) => ({
          ...
          >
            ...
            <View style={styles.ingredientRight}>
              ...
              <Text style={[styles.ingredientAmount, { color: theme.muted }]}>
                {item.amount} g
              </Text>
            </View>
          </Pressable>
